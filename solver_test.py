...
def test_solve():
    ...
    # The expected solution should be in the solutions list (order-independent)
    expected = [
    ...
    expected_sorted = sorted(expected, key=lambda s: (s.string, s.positions))
    assert any(
        sorted(solution, key=lambda s: (s.string, s.positions)) == expected_sorted
